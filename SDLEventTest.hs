...
toFrame (xdim, ydim) (x', y') = ListFrame $ map (\y -> map (\x -> if x == x' && y == y' then Pixel 0 158 130 else Pixel 238 238 238) [0 .. xdim - 1]) [0 .. ydim - 1]
pixelsaur :: (Int, Int) -> (Int, Int) -> ListFrame
pixelsaur (xdim, ydim) (x', y') = ListFrame $ map (\y -> map (\x -> if x == x' && y == y' then Pixel 0 158 130 else Pixel 238 238 238) [0 .. xdim - 1]) [0 .. ydim - 1]
...
main = do
    ...
    Sock.withSocketsDo $ runMateM (Config (fromJust $ parseAddress "134.28.70.172") 1337 dim (Just 33000) True [sdlKeyEventProvider]) eventTest (1,11) 
